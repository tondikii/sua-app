...
describe('GoogleCalendarService', () => {
  ...
  describe('buildAuthUrl', () => {
    it('should include offline access + calendar.events scope + login_hint from user email', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ email: 'tondikiag30@gmail.com' });

      const url = await service.buildAuthUrl('user-1', '/trip/abc');
      expect(mockOAuth.generateAuthUrl).toHaveBeenCalledWith(
        expect.objectContaining({
          access_type: 'offline',
          prompt: 'consent',
          scope: 'https://www.googleapis.com/auth/calendar.events',
          login_hint: 'tondikiag30@gmail.com',
        }),
      );
      expect(url).toContain('https://accounts.google.com');
    });

    it('should omit login_hint when user has no email', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ email: null });

      await service.buildAuthUrl('user-1', '/trip/abc');
      expect(mockOAuth.generateAuthUrl).toHaveBeenCalledWith(
        expect.not.objectContaining({ login_hint: expect.any(String) }),
      );
    });
    ...
    it('should throw when not configured', async () => {
      ...
      await expect(svc.buildAuthUrl('user-1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('handleCallback', () => {
    it('should store tokens and return redirect URL', async () => {
      ...
      const state = Buffer.from(JSON.stringify({ userId: 'user-1', redirect: '/trip/abc' })).toString('base64url');
  ...
  describe('createEvent', () => {
    ...
    it('should create a timed event with local wall-clock dateTime', async () => {
      ...
      const body = JSON.parse(init.body);
      expect(body.start).toEqual({ dateTime: '2026-08-15T08:30:00', timeZone: 'Asia/Jakarta' });
      expect(body.end).toEqual({ dateTime: '2026-08-17T17:45:00', timeZone: 'Asia/Jakarta' });
    });

    it('should enrich payload with tags, description, location and attendees', async () => {
      prismaMock.user.findFirst.mockResolvedValue({
        googleAccessToken: 'at',
        googleRefreshToken: null,
        googleTokenExpiresAt: new Date(Date.now() + 3600_000),
      ...
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: async () => ({ id: 'evt-4', htmlLink: null }),
      } as any) as any;
      ...
      const init = (global.fetch as jest.Mock).mock.calls[0][1];
      const body = JSON.parse(init.body);
      expect(body.summary).toBe('Lombok Trip · #pantai #liburan');
      expect(body.description).toContain('Liburan keluarga');
      expect(body.description).toContain('Hari 1:');
      expect(body.location).toBe('Lombok');
